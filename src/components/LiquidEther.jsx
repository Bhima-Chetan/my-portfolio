...
// Full fluid simulation version (heavy). Added fullScreen wrapper support like previous simplified component.
export default function LiquidEther({
  ...
  if (isMobile) {
    if (resolution > 0.35) resolution = 0.35;
    iterationsPoisson = Math.min(iterationsPoisson, 20);
    iterationsViscous = Math.min(iterationsViscous, 20);
    cursorSize = Math.min(cursorSize, 80);
  }

  useEffect(() => {
    if (!mountRef.current) return;
    // Ensure body/background doesn't block the canvas
    try {
      const b = document.body; if (b) { b.style.backgroundColor = 'transparent'; }
      const app = document.querySelector('.App'); if (app) app.style.background = 'transparent';
    } catch(e){ /* noop */ }
    console.info('[LiquidEther] mounting heavy fluid simulation');
...
`;
    ...
    class WebGLManager {
      ...
      dispose() { try { window.removeEventListener('resize', this._resize); document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); if (Common.renderer) { const canvas = Common.renderer.domElement; if (canvas && canvas.parentNode) canvas.parentNode.removeChild(canvas); Common.renderer.dispose(); } } catch (e) { /* noop */ } }
    }

  const container = mountRef.current;
  console.info('[LiquidEther] container size', container.getBoundingClientRect());
  ...
  console.info('[LiquidEther] starting render loop');
  webgl.start();
    ...
    const io = new IntersectionObserver(entries => {
      ...
      console.info('[LiquidEther] visibility change', isVisible);
      if (!webglRef.current) return;
  ...
  const ro = new ResizeObserver(() => {
    ...
    return () => {
      console.info('[LiquidEther] disposing');
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      if (resizeObserverRef.current) { try { resizeObserverRef.current.disconnect(); } catch (e) { /* noop */ } }
      if (intersectionObserverRef.current) { try { intersectionObserverRef.current.disconnect(); } catch (e) { /* noop */ } }
  ...
  // Secondary effect to update options dynamically (mirrors user-provided heavy code's pattern)
  useEffect(() => {
  ...
  const inner = <div ref={mountRef} className={`liquid-ether-container ${className || ''}`} style={mergedStyle} />;
  if (reduced) {
    return fullScreen ? <div className="liquid-ether-bg" aria-hidden="true" style={{ background:'#000', zIndex }}>{inner}</div> : inner;
  }
  return fullScreen ? <div className="liquid-ether-bg" aria-hidden="true" style={{ zIndex }}>{inner}</div> : inner;
}
